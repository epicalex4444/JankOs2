...
impl SysSegment{
    pub fn new(access: u8, flags: u8, limit: u32, base: u64) -> SysSegment {
        ...
        let mut sys_seg = SysSegment{segment_first: seg_1, segment_second: seg_2};
        sys_seg.set_base();
        sys_seg.set_limit();
        return sys_seg
    }
...
impl Segment{
    pub fn new(access: u8, flags:u8, limit: u32) -> Segment {
        ...
        return Segment{ segment_descriptor: seg}
    }
}
...
pub fn init_gdt() -> () {
    cli();
    unsafe{
        let gdt_desc = GDTDescriptor{size: size_of::<TABLE>() as u16 - 1 , offset: (&TABLE as *const TABLE) as u64 };
        lgdt(&gdt_desc);
    }
}
